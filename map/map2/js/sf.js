...
$(window).resize(function(){
	...
	$('body').off('touchend','.distancePlus').on('touchend','.distancePlus',function(e){
		e.preventDefault();
	...
	function categoriesListSearch(obj){
		$(obj).addClass('current').siblings().removeClass('current');
		var productId = $(obj).data("key");
		...
		if(productId == "all"){
			//全部
			$("#r-result li").removeClass("product-state-disabled");
		}else{
			disableProduct(resultArry,otherArry);
		}		
		$("#categoriesBox").hide();
		$("#headList li").removeClass('current').children('.iconfont').addClass('icon-kongdown').removeClass('icon-kongup');
	};
	...
	$('body').off('tap','#r-result li').on('tap','#r-result li',function(){
		...
		showFoodsList(rname, raddress, rdistance,traceId)
	});
	//点击弹窗的详情按钮进入图片展示页面
	...
	function bmapInit(){
		...
		geolocation.getCurrentPosition(function(r){
			if(this.getStatus() == BMAP_STATUS_SUCCESS){
				...
				map.addOverlay(mk);
				map.panTo(r.point);
				initCenter = new BMap.Point(r.point.lng,r.point.lat);*/
	...
	function showTotalData(marketLength,jmLength,totalLength){
		var searchSort="",$searchSort = $('#searchSort');
	...
	//显示产品列表
	function showFoodsList(rname, raddress, rdistance,traceId){
		//头部信息
		var rheadhtml = '';
			rheadhtml += '<div class="searchListHead">'
							+ '<span class="iconfont icon-jiantouBack iconBack"></span>'
		...
		$('#searchThumbBox').animate({'transform':'translateX(-100%)'},function(){
			var requestParam='{"serviceKey":"D1GHjOfkTKOo8bqdtwMBKbb1EDA1I9","param":{"nodeId":"'+traceId+'"}}';
		...
		        success: function(data, textStatus){
					...
					for(var i=0;i<data.length;i++){
						searchThumblist += '<li><a>'
											//+ '<img class="thumbimg" src="img/'+data[i].goodsId+'.png">'//动态取图片
											+ '<img class="thumbimg" src="img/2.png">'
...
function getProductSort(obj){
	...
	categoriesList.push('<div id="categoriesList" class="categoriesList"><span data-key="all" onClick="categoriesListSearch(this)">全部</span>');
	$.each(proInfo,function(key,value){
		categoriesList.push('<span data-key="'+key +'" onClick="categoriesListSearch(this)">'+ value +'</span>');
	})
...
function getDistanceSort(obj){
	...
	for (var i = 0; i < _dataArray.length; i++) {
		lists += '<li id="'+_dataArray[i].compid+'"><span class="iconfont icon-yuandian"></span><p class="storeName">'+ _dataArray[i].storeName +'</p><p class="address">'+ _dataArray[i].address +'</p><span class="distance">'+ _dataArray[i].distance + 'km'  +'</span><span class="iconfont icon-liebiao"></span></li>';
	}
	lists += '</ul>';
	$('#r-result').html(lists);
